...
import { EnvioResponseDto } from './dto/EnvioResponseDto.dto';
...
export class EnviosController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de envíos por unidad',
    type: [EnvioResponseDto],
  })
  @ApiResponse({
    status: 404,
    description: 'No se encontraron envíos para esta unidad',
  })
  @ApiResponse({ status: 500, description: 'Error interno del servidor' })
  ...
  @ApiOperation({
    summary:
      'Obtener los envíos del día para una unidad con detalles de contacto',
  })
  @ApiParam({
    name: 'id',
    description: 'ID de la unidad (vehículo)',
    type: String,
  })
  @ApiResponse({
    status: 200,
    description:
      'Lista de envíos del día para la unidad con detalles de contacto.',
  })
  @ApiResponse({
    status: 404,
    description: 'No se encontraron envíos para esta unidad en el día de hoy',
  })
  @ApiResponse({ status: 500, description: 'Error interno del servidor' })
  ...
  @Get('guia/:id')
  @ApiOperation({
    summary: 'Obtener historial de envíos para una guía específica',
  })
  @ApiParam({ name: 'id', description: 'ID de la guía', type: String })
  @ApiResponse({
    status: 200,
    description: 'Historial de envíos por guía',
    type: [EnvioResponseDto],
  })
  @ApiResponse({
    status: 404,
    description: 'No se encontró ningún envío para esta guía',
  })
  @ApiResponse({ status: 500, description: 'Error interno del servidor' })
  findByGuia(@Param('id') id: string) {
    return this.enviosService.findByGuia(id);
  }

  @Post()
  @ApiCreatedResponse({
    description: 'El envío fue creado correctamente.',
    type: EnvioResponseDto,
  })
  ...
  @ApiOperation({
    summary:
      'Actualiza el estado de los envíos del día de hoy de "pendiente" a "en_ruta" para una unidad específica.',
  })
  @ApiParam({
    name: 'id',
    description: 'ID de la unidad (vehículo)',
  ...
  @ApiOkResponse({
    description: 'Envíos actualizados a "en_ruta" correctamente.',
    schema: { example: { updated: 5 } },
  })
  @ApiNotFoundResponse({
    description:
      'No se encontraron envíos pendientes para la unidad y el día de hoy.',
  })
  ...
  @ApiParam({
    name: 'id',
    description: 'ID del envío a marcar como fallido',
    type: String,
  })
  @ApiOkResponse({
    description: 'El envío ha sido marcado como fallido.',
    type: EnvioResponseDto,
  })
  ...
  marcarComoFallido(
    ...
    return this.enviosService.marcarComoFallido(
      id,
      falloEnvioDto.motivo_fallido,
    );
  }
  ...
  @ApiParam({
    name: 'nuevoEstado',
    type: String,
    description: 'Nuevo estatus (pendiente, en_ruta, entregado, fallido)',
  })
  @ApiParam({
    name: 'nombreReceptor',
    type: String,
    description: 'Es el nombre de quien lo recibio',
  })
  ...
  async actualizarEstado(
    ...
    const actualizado = await this.enviosService.actualizarEstatus(
      id,
      estado,
      nombre_receptor,
    );
