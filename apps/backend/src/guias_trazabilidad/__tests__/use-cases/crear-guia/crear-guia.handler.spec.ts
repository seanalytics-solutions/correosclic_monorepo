...
jest.mock(
  '../../../application/use-cases/crear-guia/mapper/crear-guia.mapper',
  () => ({
  ...
  describe('execute', () => {
    it('should create a guia successfully with nacional service', async () => {
      ...
      const command = new CrearGuiaCommand(
        ...
        GuiasTrazabilidadTestSetup.mockData.validCrearGuiaCommand()
          .tipoServicio as any,
      );
      ...
      expect(
        repositoryMocks.GUIAREPOSITORYINTERFACE.save,
      ).toHaveBeenCalledTimes(1);
      expect(
        repositoryMocks.QR_GENERATOR_REPOSITORY.generarQRComoDataURL,
      ).toHaveBeenCalledTimes(1);
      expect(
        repositoryMocks.PDF_GENERATOR_REPOSITORY_INTERFACE
          .generarGuiaPDFNacional,
      ).toHaveBeenCalledTimes(1);
    });
    ...
    it('should throw InternalServerErrorException when remitente coordinates fail', async () => {
      ...
      repositoryMocks.GOOGLE_GEOCODE_REPOSITORY_INTERFACE.obtenerCoordenadas.mockResolvedValueOnce(
        Result.failure('Error getting coordinates'),
      );
    ...
    it('should throw InternalServerErrorException when destinatario coordinates fail', async () => {
      ...
      repositoryMocks.GOOGLE_GEOCODE_REPOSITORY_INTERFACE.obtenerCoordenadas
        .mockResolvedValueOnce(
          Result.success(GuiasTrazabilidadTestSetup.mockData.mockCoordenadas()),
        )
    ...
    it('should throw InternalServerErrorException when QR generation fails', async () => {
      ...
      repositoryMocks.QR_GENERATOR_REPOSITORY.generarQRComoDataURL.mockResolvedValue(
        Result.failure('QR generation failed'),
      );
    ...
    it('should throw InternalServerErrorException when PDF generation fails', async () => {
      ...
      repositoryMocks.PDF_GENERATOR_REPOSITORY_INTERFACE.generarGuiaPDFNacional.mockResolvedValue(
        Result.failure('PDF generation failed'),
      );
