...
export class ObtenerGuiaPorNumeroQueryHandler implements IQueryHandler<ObtenerGuiaPorNumeroQuery> {
  constructor(
    @Inject(GUIA_READ_REPOSITORY)
    private readonly guiaReadRepository: GuiaReadRepositoryInterface,
  ) {}

  async execute(query: ObtenerGuiaPorNumeroQuery): Promise<Result<TrazabilidadReadModel>> {
    try {
      const guia = await this.guiaReadRepository.findByNumeroRastreo(query.numeroRastreo);
      
      if (!guia) {
        return Result.failure(`Guía con número ${query.numeroRastreo} no encontrada`);
      }
